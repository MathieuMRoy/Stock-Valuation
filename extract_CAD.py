...
def get_financial_data_secure(ticker):
    try:
        ...
        try:
            ...
            # Calcul manuel pour éviter le bug des 3 milliards
        ...
        # 3. INFO (Extraction des variables de croissance)
        try:
            full_info = stock.info
            sector = full_info.get('sector', 'Default')
            # On prend les taux de croissance actuels
        ...
        # On retourne un dictionnaire PROPRE (Pas d'objets yfinance, juste des données)
        return {
            "bs": bs, "inc": inc, "cf": cf, 
...
if not ticker_input:
    ...
    if data is None:
        ...
        shares = data['shares_calc'] # Calculé via Market Cap
        if shares <= 1: 
            shares = data['shares_info'] # Fallback info
        if shares <= 1: 
            shares = 1 
            st.warning("⚠️ Warning: Share count unavailable. Valuation may be wrong.")
        ...
        # --- DISPLAY HELP (AVEC LA COMPARAISON V47) ---
        with st.expander(f"💡 Help: {bench_data['name']} vs {ticker_final}", expanded=True):
            st.write(f"**Peers:** {bench_data['peers']}")
            
            # Ici on affiche : Benchmark (Gros) et Actuel (Petit Delta)
            c1, c2, c3, c4 = st.columns(4)
            
            # Note: On compare les Benchmarks (prévisionnels 5 ans) aux données actuelles (YoY)
            c1.metric("Sales Gr.", f"{bench_data['gr_sales']*100:.0f}%", delta=f"{data['rev_growth']*100:.1f}% Actual")
            c2.metric("EPS Gr.", f"{bench_data['gr_eps']*100:.0f}%", delta=f"{data['eps_growth']*100:.1f}% Actual")
