...
# --- CSS HACK (POUR CACHER LES ERREURS ROUGES MOCHES) ---
st.markdown("""
    <style>
...
def get_financial_data_secure(ticker):
    """Récupère UNIQUEMENT les données brutes (pas d'objets complexes)"""
    try:
        ...
        try:
            ...
            # On calcule les actions nous-mêmes pour éviter le bug des 3 milliards
        ...
        # 3. INFO (On prend juste ce qu'il faut)
        try:
        ...
        # On retourne un dictionnaire PROPRE (Texte et Chiffres seulement)
        return {
            "bs": bs, "inc": inc, "cf": cf, 
            "price": current_price, "shares_calc": shares_calc,
            "sector": sector, "shares_info": shares_info, "eps": eps,
...
if ticker_final:
    ...
    if data is None:
        ...
        # On priorise le calcul (Market Cap / Prix) car plus robuste que l'info Yahoo qui bug
        shares = data['shares_calc']
        if shares <= 1: 
            shares = data['shares_info'] # Fallback
        if shares <= 1: 
            shares = 1 # Avoid crash but warn
            st.warning("⚠️ Warning: Could not detect share count. Valuation may be wrong.")
        ...
        with st.expander(f"💡 Help: {bench_data['name']} vs {ticker_final}", expanded=True):
            ...
            c5.metric("Sales Gr. (YoY)", f"{data['rev_growth']*100:.1f}%")
            c6.metric("EPS Gr. (YoY)", f"{data['eps_growth']*100:.1f}%")
