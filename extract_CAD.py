...
with st.sidebar:
    ...
    # Ajout du champ pour la clé Groq
...
def _robust_price(stock: yf.Ticker, ticker: str) -> float:
    price = 0.0
    try:
        df = yf.download(ticker, period="5d", interval="1d", progress=False, threads=False, auto_adjust=False)
        if df is not None and not df.empty:
            ...
            if "Close" in df.columns and df["Close"].dropna().shape[0] > 0:
                ...
                price = float(df["Adj Close"].dropna().iloc[-1])
    except Exception:
        pass
    if price <= 0:
        try:
            ...
            if hist is not None and not hist.empty:
                if "Close" in hist.columns and hist["Close"].dropna().shape[0] > 0:
...
def get_growth_manual(df: pd.DataFrame, keys: list) -> float:
    try:
        ...
        row = None
        for key in keys:
            # Recherche flexible (ex: "Revenue" trouve "Total Revenue")
        ...
        # Secours : Croissance trimestrielle
        elif len(vals) >= 2:
            current = vals[0]
            prev = vals[1]
            if prev != 0:
...
def get_financial_data_secure(ticker: str) -> dict:
    ...
    try:
        ...
        # PRICE
        current_price = _robust_price(stock, ticker)
        out["price"] = float(current_price or 0)
        ...
        out["shares_info"] = float(full_info.get("sharesOutstanding", 0) or 0)
        ...
        shares_calc = 0.0
        if market_cap and out["price"] and out["price"] > 0:
            shares_calc = float(market_cap) / float(out["price"])
        out["shares_calc"] = float(shares_calc or 0)
...
def calculate_valuation(
    ...
    if current_fcf <= 0 or shares <= 0 or wacc_val <= 0:
        price_dcf = 0.0
    else:
        ...
        price_dcf = ((pv_fcf + (terminal_val / ((1 + wacc_val) ** 5))) + cash - debt) / max(shares, 1)

    if revenue <= 0 or shares <= 0:
        price_sales = 0.0
    else:
        price_sales = (((revenue * ((1 + gr_sales) ** 5)) * ps_target) / max(shares, 1)) / (1.10 ** 5)
...
# =========================================================
# 6) AI ANALYST (MODE EXPERT / COVERAGE / SENIOR) - GROQ
# =========================================================
...
if mode == "Stock Analyzer":
    ...
    if shares <= 1:
        shares = 1.0
        st.warning(" Warning: Share count unavailable (fallback=1).")
    ...
    with tabs[8]:
        ...
        if st.button(" Générer le Rapport Complet"):
            ...
            with st.spinner("L'analyste rédige le rapport (c'est un peu plus long)..."):
                # Passe la clé API depuis la sidebar
