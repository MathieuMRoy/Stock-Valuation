...
# --- SIDEBAR: BOUTON RESET (ESSENTIEL) ---
with st.sidebar:
    if st.button("🗑️ Reset Cache (Fix Errors)"):
        st.cache_data.clear()
        st.rerun()
    st.caption("Cliquez ici si 'Data not found' ou erreur rouge.")
...
# --- 2. DATA FUNCTIONS (FIXED) ---
@st.cache_data(ttl=3600)
def get_financial_data(ticker):
    try:
        stock = yf.Ticker(ticker)
        
        # 1. PRIX & SHARES (Méthode Rapide & Robuste)
        # On utilise fast_info qui est plus fiable que .info
        try:
            current_price = stock.fast_info['last_price']
            market_cap = stock.fast_info['market_cap']
            # CALCUL MANUEL DES ACTIONS (Anti-Bug 3 Milliards)
            ...
            shares = 0 # On essaiera de le trouver ailleurs
        ...
        # 3. INFO (Optionnel, juste pour le secteur)
        try:
            info = stock.info
        except:
            info = {}
        
        # On retourne un dictionnaire SIMPLE (Pas d'objets complexes)
        return {
            "bs": bs, 
            "inc": inc, 
            "cf": cf, 
            "info": info, 
            "price": current_price,
            "shares_calc": shares # On passe le calcul manuel
        }
...
if not ticker_input:
    ...
    if data is None:
        ...
        shares_calc = data['shares_calc'] # Le chiffre calculé

        # 1. SHARES LOGIC (CRITICAL)
        # On utilise le chiffre calculé (Market Cap / Price) car il est plus fiable que .info['sharesOutstanding']
        shares = shares_calc
        if shares <= 1:
             # Dernier recours si le calcul a échoué
             shares = info.get('sharesOutstanding', 1)
        
        if shares <= 1:
            st.error("⚠️ ERREUR CRITIQUE : Impossible de trouver le nombre d'actions. Les calculs seront faux.")
            shares = 1 # Pour éviter division par zéro, mais le résultat sera aberrant
